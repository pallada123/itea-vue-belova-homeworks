...
export default {
  ...
  methods: {
    ...
    /**
     * отсекает от пути текущего маршрута часть, касающаюся текущей страницы,
     ...
     * чтобы после закрытия попапа в любом случае увитеть имено ту страницу,
     * которую мы видели под попапом, даже если в истории "предыдущей" страницы нет.)
     *
     * после перенаправления вызывается метод очистки переменной SinglePost
     * (чтобы при открытии второго и далее попапа не видеть, как один текст заменяется надругой)
     */
    goPrev() {
      let index = this.$route.path.indexOf('/post-' + this.$route.params.postId);
